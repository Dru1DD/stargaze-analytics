...
import { formatIpfsLink } from "@/helpers/format-ipfs";
...
export const BubbleMap: React.FC<BubbleMapProps> = ({
  ...
  const initTooltip = useCallback(() => {
    ...
    return (d3Tip as any)()
      ...
      .html((d: any) => {
        ...
        return `
                  <div style="position: sticky; padding: 20px; max-width: 400px; background: black; color: white; border: 1px solid white; border-radius: 12px;">
                      ${
                        isOwner
  ...
  const renderBubbleMap = useCallback(() => {
    ...
    const highlightColor = "rgba(239, 90, 46, 0.8)";
    ...
    const node = container
      ...
      .on("mouseout", function (event, d) {
        const fillColor = d.data.count > 400 ? highlightColor : baseColor;
    ...
    node
      .append("circle")
      .attr("r", (d: any) => d.r)
      .attr("fill", (d: any) => (d.data.count > 400 ? highlightColor : baseColor))
    ...
    tooltipElements.forEach((el) => {
      el.setAttribute(
        "style",
        "pointer-events: none; z-index: 1000; font-size: 16px; padding: 10px; position: fixed"
      );
    });
